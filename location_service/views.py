...
from diagnostic_service.models import Inspection
...
def client_dashboard(request):
    ...
    # Dernières inspections (3 max) - On filtre par les car_id des réservations de l'utilisateur
    user_car_ids = Booking.objects.filter(user=request.user).values_list('car_id', flat=True)
    inspections = Inspection.objects.filter(car_id__in=user_car_ids).order_by('-id')[:3]
...
def client_reports(request):
    ...
    user_car_ids = Booking.objects.filter(user=request.user).values_list('car_id', flat=True)
    inspections = Inspection.objects.filter(car_id__in=user_car_ids).distinct().order_by('-created_at')
    
    # Statistiques simples
    total_scans = Inspection.objects.count()
...
def upload_image(request, car_id=None):
    ...
    if request.method == "POST":
        ...
        # APPEL AU MICROSERVICE DIAGNOSTIC (Communication inter-services)
        try:
            diagnostic_url = "http://localhost:8000/api/diagnostic/run/"
            response = requests.post(diagnostic_url, files={'image': image}, data={'car_id': car_id})
            result = response.json()
            return JsonResponse(result)
        except Exception as e:
            # Fallback si le microservice est sur le même projet
            from diagnostic_service.models import Inspection
            inspection = Inspection.objects.create(car_id=car_id, image_input=image)
            return JsonResponse({"status": "partial_success", "id": inspection.id})
...
def report_detail(request, pk):
    ...
    if not ref_dets:
        pickup = Inspection.objects.filter(car=inspection.car_id, inspection_type='pickup').exclude(pk=inspection.pk).first()
        if pickup:
...
def book_car(request, pk):
    ...
    if request.method == "POST":
        ...
        if form.is_valid():
            ...
            booking.save()
            # Redirection vers le paiement au lieu du dashboard direct
            return redirect("inspections:payment_gate", booking_id=booking.id)
        else:
            for error in form.errors.values():
                messages.error(request, error)
    else:
...
def payment_gate(request, booking_id):
    """Page intermédiaire de paiement."""
    booking = get_object_or_404(Booking, id=booking_id)
    amount = booking.car.price_per_day * booking.duration_days
    return render(request, "inspections/payment_page.html", {
        "booking": booking,
        "amount": amount
    })

@client_required
def process_payment_gate(request):
    """Communique avec le microservice Payment."""
    if request.method == "POST":
        booking_id = request.POST.get('booking_id')
        amount = request.POST.get('amount')
        
        # APPEL AU MICROSERVICE PAYMENT
        try:
            payment_url = "http://localhost:8000/api/payment/process/"
            ...
            if result.get('status') == 'success':
                ...
                booking.status = 'validated' # Marquer comme validé après paiement
                booking.save()
                messages.success(request, "Paiement réussi ! Votre réservation est validée.")
                return redirect("inspections:dashboard")
            else:
                messages.error(request, "Le paiement a échoué.")
        except Exception as e:
            # Fallback local si le microservice n'est pas lancé séparément
            ...
            booking.save()
            messages.success(request, "Paiement validé (Mode local).")
            return redirect("inspections:dashboard")
...
def admin_notifications(request):
    """Vue pour les alertes et notifications système."""
